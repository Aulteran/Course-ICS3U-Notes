...
def new_entry(movie_name, director, gross_sales):
    '''creates a new entry in movies db'''
    movies[movie_name] = [director, gross_sales]

def call_entry(movie_name):
    '''shows details for a specific movie'''
    for movie, moviedata in movies.items():
        if movie == movie_name:
            print(f"Movie Name: {movie}\nDirector: {moviedata[0]}\nGross Sales: {moviedata[1]}")
            return
    print("Entry not found.")

def change_entry(movie_name, new_director, new_sales):
    '''changes movie item in db'''
    for movie, moviedata in movies.items():
        if movie == movie_name:
            moviedata[0] = new_director
            moviedata[1] = new_sales
...
while True:
    ...
    if menu_select == 1:
        ...
        change_entry(name, director, sales)
    elif menu_select == 4:
        break
    else:
